...
import type { QuizPromptConfig } from '@/db/schema/database'
import {
  createQuizPromptConfig,
  deleteQuizPromptConfig,
  duplicateQuizPromptConfig,
  getAllQuizPromptConfigs,
  saveQuizPromptConfig,
  setActiveQuizPromptConfig
} from '@/db/repositories/quiz-prompt-config.repository'

export function useQuizPromptConfigs(showToast: (message: string, type?: 'success' | 'error') => void) {
  const configs = ref<QuizPromptConfig[]>([])
  ...
  async function loadConfigs() {
    try {
      configs.value = await getAllQuizPromptConfigs()
  ...
  async function createNewConfig() {
    try {
      const newConfig = await createQuizPromptConfig({
        name: 'New Quiz Prompt Config',
        content: '',
        isActive: false
      })
  ...
  async function saveConfig(id: string, name: string, content: string, isActive: boolean) {
    ...
    try {
      await saveQuizPromptConfig({ id, name, content, isActive })
  ...
  async function setAsActive(id: string) {
    try {
      await setActiveQuizPromptConfig(id)
  ...
  async function duplicateConfig(id: string) {
    try {
      const duplicate = await duplicateQuizPromptConfig(id)
      if (duplicate) {
  ...
  async function performDelete() {
    ...
    try {
      await deleteQuizPromptConfig(idToDelete)
